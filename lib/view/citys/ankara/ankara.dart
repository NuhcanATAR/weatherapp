...
class ankaraWeatherScreen extends StatefulWidget {
  const ankaraWeatherScreen({super.key});

  @override
  State<ankaraWeatherScreen> createState() => _ankaraWeatherScreenState();
}

// ignore: camel_case_types
class _ankaraWeatherScreenState extends State<ankaraWeatherScreen> {
  String ankaraCity = "";
  List<WeatherAnkara> weatherAnkaraList = [];

  @override
  void initState() {
    super.initState();
    getWeatherAnkaraData("Ankara");
  }

  void getWeatherAnkaraData(String cityAnkaraData) async {
    ApiCityServices.getWeatherAnkaraCity(cityAnkaraData).then((data) {
      ...
      if (resultBody['success'] == true) {
        setState(() {
          ...
          weatherAnkaraList =
              result.map((weatherData) => WeatherAnkara(weatherData)).toList();
        });
      }
    });
  }

  String buildWeatherListAnkaraText(String day) {
  ...
  Widget build(BuildContext context) {
    ...
    if (weatherAnkaraList.isEmpty) {
      return errorLoadingScreen;
    } else {
      String durum = weatherAnkaraList[0].description;
      String sonuc = "";
                                    ...
                                    FadeInUp(
                                      child: SizedBox(
                                        width:
                                            MediaQuery.of(context).size.width,
                                        child: Text(
                                          "${double.parse(weatherAnkaraList[0].degree).round().toString()}°C",
                                    ...
                                    FadeInUp(
                                      child: SizedBox(
                                        width:
                                            MediaQuery.of(context).size.width,
                                        child: Text(
                                          weatherAnkaraList[0].description,
                                          style: GoogleFonts.nunito(
                                            textStyle: Theme.of(context)
                                                .textTheme
                                                .titleLarge!
                                                          ...
                                                          child: Row(
                                                            ...
                                                            children: <Widget>[
                                                              Flexible(
                                                                ...
                                                                flex: 1,
                                                                child: Text(
                                                                  "${double.parse(weatherAnkaraList[index + 1].min).round()}°",
                                                                  style:
                                                                      GoogleFonts
                                                                          .nunito(
                                                                    textStyle: Theme.of(
